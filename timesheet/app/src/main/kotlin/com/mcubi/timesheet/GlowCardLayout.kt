...
class GlowCardLayout @JvmOverloads constructor(
    ...
    companion object {
        // 25s cycle: 0-15s hold full, 15-20s fade out, 20-25s fade in
    ...
    private val breathingOverlay = BreathingOverlay(context).also {
        it.visibility = GONE; addView(it, LayoutParams(0, 0))
    }
    ...
    private var breathPending = false
    ...
    override fun onFinishInflate() {
        ...
        removeView(pulseOverlay); removeView(breathingOverlay)
    ...
    fun startBreathing() {
        ...
        val l = ValueAnimator.AnimatorUpdateListener { anim ->
            ...
            if (pulsePending && b < 0.2f) {
                pulsePending = false; isPulsing = true
    ...
    fun stopBreathing() {
        ...
        breathListener = null; breathingOverlay.visibility = GONE
    }
        ...
        if (pulseListener == null) {
            val l = ValueAnimator.AnimatorUpdateListener { anim ->
                ...
                if (breathPending && p < 0.08f && lastPulseProgress > 0.85f) {
                    breathPending = false; isPulsing = false
    ...
    fun stopPulse() {
        if (!isPulsing && !pulsePending) return
        pulsePending = false
        if (isPulsing) breathPending = true
    ...
    fun setAlertMode(alert: Boolean) {
        ...
        else { pulseOverlay.setAccent(glowColor); stopPulse() }
    }
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        if (changed) { breathingOverlay.buildPath(w, h, glowPad); pulseOverlay.buildPath(w, h, glowPad) }
    }
    ...
    private inner class BreathingOverlay(ctx: Context) : View(ctx) {
        ...
        private val alphaFloor = 18
        private val alphaCeil  = 75
        private val glowPaint  = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style = Paint.Style.STROKE; strokeWidth = 16f * density
        ...
        fun buildPath(cardW: Int, cardH: Int, pad: Int) {
            ...
            path.addRoundRect(RectF(off+inset,off+inset,cardW+off-inset,cardH+off-inset), cornerPx,cornerPx, Path.Direction.CW)
    ...
    private inner class PulseOverlay(ctx: Context) : View(ctx) {
        ...
        private val outerGlowPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style=Paint.Style.STROKE; strokeWidth=18f*density
            maskFilter=BlurMaskFilter(14f*density,BlurMaskFilter.Blur.NORMAL)
        }
        private val haloPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
            style=Paint.Style.STROKE; strokeWidth=strokePx*4f
        ...
        init { setLayerType(LAYER_TYPE_SOFTWARE, null) }
        fun setAccent(color: Int) { outerGlowPaint.color=color; haloPaint.color=color; corePaint.color=color }
        ...
        fun buildPath(cardW: Int, cardH: Int, pad: Int) {
            ...
            val r=Color.red(glowColor);val g=Color.green(glowColor);val b=Color.blue(glowColor)
            outerGlowPaint.color=Color.argb(255,r,g,b); haloPaint.color=Color.argb(255,r,g,b); corePaint.color=Color.argb(255,r,g,b)
        }
        override fun onDraw(canvas: Canvas) {
            val len = pm.length; if (len==0f) return
            val headPos = progress*len; val N=64
            for (i in 0 until N) {
                val behind = i.toFloat()/N*tailFraction
                val t = 1f-(behind/tailFraction)
                val alpha = (t*t*t*t*255f).toInt().coerceIn(0,255); if (alpha<2) continue
                val segLen=len*tailFraction/N
                val sEnd=((headPos-len*behind)+len*10f)%len; val sStart=((sEnd-segLen)+len)%len
                val seg=Path()
                if (sStart<sEnd) { pm.getSegment(sStart,sEnd,seg,true) }
                else { pm.getSegment(sStart,len,seg,true); val w=Path(); pm.getSegment(0f,sEnd,w,true); seg.addPath(w) }
                val outerT=(t-0.6f).coerceAtLeast(0f)/0.4f; val outerAlpha=(outerT*outerT*130f).toInt()
                if (outerAlpha>1) { outerGlowPaint.alpha=outerAlpha; canvas.drawPath(seg,outerGlowPaint) }
                haloPaint.alpha=(alpha*0.45f).toInt(); corePaint.alpha=alpha
                canvas.drawPath(seg,haloPaint); canvas.drawPath(seg,corePaint)
            }
        }
    }
}