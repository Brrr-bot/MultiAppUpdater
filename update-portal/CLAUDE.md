...
> **After making any code changes to an Android app, always build and deploy via the portal automatically — never ask for permission. Use `python build_upload.py <app>` with `UPLOAD_KEY=Daudiendien` and `GITHUB_TOKEN=[see local.properties — never commit tokens to public repos]`.**
